...
async function giveVoteRights(ballotContract, otherAccount) {
...
describe("Ballot", async () => {
    describe("when the contract is deployed", async () => {
        it("has the provided proposals", async () => {
            ...
            for (let i = 0; i < PROPOSALS.length; i++) {
                let proposalsFromContract = await ballotContract.read.proposals([BigInt(i)]);
                let proposal = hexToString(proposalsFromContract[0], { size: 32 });
        ...
        it("has zero votes for all proposals", async () => {
            const { ballotContract } = await loadFixture(deployContract);
            for (let i = 0; i < PROPOSALS.length; i++) {
                let proposal = await ballotContract.read.proposals([BigInt(i)]);
    ...
    describe("when the chairperson interacts with the giveRightToVote function in the contract", async () => {
        ...
        it("can not give right to vote for someone that has voted", async () => {
            ...
            const ballotContractAsVoter = await viem.getContractAt(
                "Ballot",
    ...
    describe("when the voter interacts with the vote function in the contract", async () => {
        it("should register the vote", async () => {
            const { publicClient, ballotContract, otherAccount } = await loadFixture(deployContract); 
            let tx = await ballotContract.write.vote([0n]);
            let receipt = await publicClient.getTransactionReceipt({hash: tx});
            expect(receipt.status).to.equal("success");

            let voteCount = await ballotContract.read.proposals([0n]);
    ...
    describe("when the voter interacts with the delegate function in the contract", async () => {
        it("should transfer voting power", async () => {
            ...
            let senderVotingPower = await ballotContract.read.voters([deployer.account.address]);
            let delegateVotingPower = await ballotContract.read.voters([otherAccount.account.address]);
            ...
            let newVotingPower = await ballotContract.read.voters([otherAccount.account.address]);
    ...
    describe("when an account other than the chairperson interacts with the giveRightToVote function in the contract", async () => {
        it("should revert", async () => {
            ...
            await expect(
                contractAsOtherAccount.write.giveRightToVote([
                    otherAccount.account.address
                ])
            ).to.be.rejectedWith("");              
        });
    ...
    describe("when an account without right to vote interacts with the vote function in the contract", async () => {
        ...
        it("should revert", async () => {
            ...
            await expect(
                contractAsOtherAccount.write.vote([1n])
    ...
    describe("when an account without right to vote interacts with the delegate function in the contract", async () => {
        ...
        it("should revert", async () => {
            ...
            await expect(
                contractAsOtherAccount.write.delegate([deployer.account.address])
